    ...
    if (sig_code.toInt() == MACRO_CHAT_SERVER){
        ...
        case MACRO_GET_GOODS_INFO:{
            if (key_ret == MACRO_GET_GOODS_INFO_SUCCESS){
                emit sig_get_all_goods_info_success(data_array);
            }else{
        ...
        case MACRO_USER_PAY:{
            if (key_ret == MACRO_PAY_SUCCESS){
                emit sig_user_pay_success();
            }else{
                emit sig_user_pay_fail();
            }break;
        }; // 用户支付
        case MACRO_GET_ADMIN_CODE:{
            if (key_ret == MACRO_GET_CODE_OK){
                qDebug() << "sig" << key_data;
                emit sig_admin_code_ok(key_data);
            }else{
                emit sig_admin_code_fail();
            }break;
        }; // 获取管理员授权码
        case MACRO_GET_PAY_CODE:{
            if (key_ret == MACRO_PAY_CODE_OK){
                emit sig_get_pay_code_ok(key_data);
            }else{
                emit sig_get_pay_code_fail(key_data);
            }
        }; // 获取支付二维码
        }
    }
...
void NetManager::Send_Manage_Goods_Info(QMap<QString, int> manage_info)
{
    ...
    main_obj.insert("key_head", MACRO_REQ_PACK);
...
void NetManager::Get_Admin_Code()
{
    QJsonDocument manage_doc;
    QJsonObject main_obj;
    ...
    main_obj.insert("key_head", MACRO_REQ_PACK);
    main_obj.insert("key_sig", MACRO_CHAT_SERVER);
    main_obj.insert("key_func", MACRO_GET_ADMIN_CODE);
    manage_doc.setObject(main_obj);
    tcp_messager->write(manage_doc.toJson());
}
